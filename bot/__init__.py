...
bot = tgClient("bot", bot_token=BOT_TOKEN, api_id=TELEGRAM_API, api_hash=TELEGRAM_HASH)
...
async def help(client, message):
    await message.reply("Here is the help text.")

@bot.on_message(filters.command("add_torrent") & filters.private)
async def add_torrent(client, message):
    if len(message.command) != 2:
        await message.reply("Usage: /add_torrent <torrent_url>")
        return
    torrent_url = message.command[1]
    await add_aria2c_torrent(torrent_url, DOWNLOAD_DIR)
    await message.reply("Torrent added!")
...
async def main():
    ...
    # Scheduled jobs
    scheduler.add_job(check_download_status, 'interval', seconds=DOWNLOAD_STATUS_UPDATE_INTERVAL)
...
async def check_download_status():
    # Code to check and update the download status
    pass

async def refresh_torrent_info():
    # Code to refresh torrent information
    pass

async def cleanup_old_tasks():
    # Code to clean up old tasks
    pass
